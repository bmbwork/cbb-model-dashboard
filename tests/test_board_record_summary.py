...
def test_cfb_style_cbb_summary_is_six_cards_and_final_only():
    ...
    assert "saved sportsbook picks" in values["Spread record"][1]
...
def test_missing_spread_and_pending_games_never_become_losses():
    ...
    assert "awaiting graded spread results" in values["Spread record"][1]


def test_model_line_fallback_populates_spread_record_but_is_not_labeled_ats():
    b=pd.DataFrame([{
        "Away Team":"Illinois","Home Team":"UConn","Model Pick":"Illinois","Win Probability":.687,"_win_prob":.687,
        "Fair Spread":-4.3,"_grade_eligible":True,"_ml_correct":False,"_spread_correct":pd.NA,
        "_final_away":62,"_final_home":71,"_filter_best_ml":pd.NA,"_filter_best_spread":pd.NA,
    }]).astype({"_ml_correct":"boolean","_spread_correct":"boolean"})
    values={label:(value,sub) for label,value,sub in summary_cards(b)}
    assert values["Spread record"][0]=="0-1"
    assert values["Spread record"][1]=="frozen model-line results; not ATS wagers"


def test_saved_sportsbook_grade_takes_priority_over_model_line_fallback():
    b=pd.DataFrame([{
        "Away Team":"Illinois","Home Team":"UConn","Model Pick":"Illinois","Win Probability":.687,"_win_prob":.687,
        "Fair Spread":-4.3,"_grade_eligible":True,"_ml_correct":False,"_spread_correct":True,
        "_final_away":62,"_final_home":71,"_filter_best_ml":-180,"_filter_best_spread":10.0,
    }]).astype({"_ml_correct":"boolean","_spread_correct":"boolean"})
    values={label:(value,sub) for label,value,sub in summary_cards(b)}
    assert values["Spread record"][0]=="1-0"
    assert "saved sportsbook picks" in values["Spread record"][1]
