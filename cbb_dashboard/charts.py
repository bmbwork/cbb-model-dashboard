...
def confidence_chart(board: pd.DataFrame, top_n: int = 25) -> go.Figure:
    ...
    fig.update_xaxes(title="Model win chance", ticksuffix="%", range=[50, max(100, float((data["_win_prob"]*100).max()) + 2)])
    return _layout(fig, f"Top {min(top_n, len(data))} strongest model picks")
...
def team_comparison_chart(row: pd.Series) -> go.Figure:
    ...
    metrics = ["Offense rating", "Defense rating", "Overall rating"]
    ...
    fig.update_yaxes(title="Opponent-adjusted rating")
    return _layout(fig, "Team strength profile · lower defense rating is better")
...
def performance_trend(history: pd.DataFrame) -> go.Figure:
    ...
    fig.update_yaxes(title="Average margin miss (points)")
    return _layout(fig, "Average margin miss by slate")
...
def calibration_chart(buckets: pd.DataFrame) -> go.Figure:
    ...
    fig.update_xaxes(title="Average predicted win chance", ticksuffix="%", range=[50,100])
    fig.update_yaxes(title="Observed win rate", ticksuffix="%", range=[0,100])
    return _layout(fig, "Does model confidence match actual win rate?")
